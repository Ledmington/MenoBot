...
def update_all_prices_command(update, context):
    ...
    if len(current_user.get_interesting_cards()) == 0:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="You have no cards."
        )
        return
...
def update_price_command(update, context):
    ...
    if not re.match(r"\d+", input_string):
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="Invalid number."
        )
        return
...
def add_command(update, context) -> int:
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="Please type the string to search your card.",
    )
...
def add_card(update, context) -> int:
    ...
    message = (
        utils.compose_list(current_user.retrieved_cards, with_index=True)
        + "\nPlease type a number from 1 to "
        + str(len(current_user.retrieved_cards))
        + " to choose that card."
    )
...
def save_new_card(update, context) -> int:
    ...
    new_card = Card(
        current_user.retrieved_cards[selected_card][0],
        utils.CardMarketURLs["base"] + current_user.retrieved_cards[selected_card][1],
    )
    current_user.add_card(new_card, update, context)

    message = (
        "<b>" + current_user.retrieved_cards[selected_card][0] + "</b> added to list."
    )
...
def remove_card(update, context) -> int:
    ...
    if len(current_user.get_interesting_cards()) == 0:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="You have no cards to remove."
        )
        return
